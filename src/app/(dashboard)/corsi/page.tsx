...
export default async function CorsiPage() {
  ...
  const statusLabels = t.corsi.statusLabels
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto space-y-8">
      <div>
        ...
        <p className="text-verde-500 mt-1 text-sm">{t.corsi.pageSubtitle}</p>
      </div>

      {corsi.length === 0 ? (
        <div className="text-center py-20">
          <Video size={48} className="text-verde-800 mx-auto mb-4" />
          <p className="text-verde-500">{t.corsi.emptyState}</p>
        </div>
      ) : (
                ...
                <div className="flex items-start justify-between gap-4 flex-wrap">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap mb-1">
                        ...
                        <span className={`px-2 py-0.5 rounded text-[10px] font-bold border ${STATUS_COLORS[corso.status] ?? ''}`}>
                          {statusLabels[corso.status as keyof typeof statusLabels] ?? corso.status}
                        </span>
                      </div>
                  ...
                  {accessible && corso.meeting_url && corso.status === 'published' ? (
                    <a href={corso.meeting_url} target="_blank" rel="noopener noreferrer"
                      ...
                      <ExternalLink size={12} /> {t.corsi.joinBtn}
                    </a>
                  ) : !accessible ? (
