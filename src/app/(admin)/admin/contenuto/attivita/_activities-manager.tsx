...
import { Plus, Pencil, Trash2, Gamepad2, Loader2, X, ToggleLeft, ToggleRight, Upload, CheckCircle2, ExternalLink, ChevronDown, ChevronUp } from 'lucide-react'
...
function QuizBuilder({ questions, onChange }: { questions: QuizQuestion[]; onChange: (qs: QuizQuestion[]) => void }) {
  ...
  function add() {
    const qs = [...questions, emptyQuestion()]
    onChange(qs)
  ...
  function update(i: number, q: Partial<QuizQuestion>) {
    const qs = [...questions]
    qs[i] = { ...qs[i], ...q }
    onChange(qs)
  }
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        <label className="block text-xs text-verde-500">
        ...
        <div key={i} className="rounded-xl border border-verde-800/30 bg-verde-950/20 overflow-hidden">
          ...
          >
            <span className="text-xs text-verde-400 font-medium truncate">
              {i + 1}. {q.text || 'Pregunta sin título'}
            </span>
          ...
          {expanded === i && (
            <div className="px-3 pb-3 space-y-3 border-t border-verde-900/30">
              <div className="pt-3">
                <label className="block text-[10px] text-verde-600 mb-1">Texto de la pregunta</label>
                <textarea
                  value={q.text}
                  onChange={e => update(i, { text: e.target.value })}
                  rows={2}
                  placeholder="¿Cómo se dice 'gracias' en italiano?"
                  className="w-full px-3 py-2 rounded-lg bg-verde-950/50 border border-verde-800/40 text-verde-200 text-xs focus:outline-none focus:border-verde-600 resize-none"
                />
              </div>

              <div>
                <label className="block text-[10px] text-verde-600 mb-1">Opciones (marca la correcta)</label>
                <div className="space-y-1.5">
                  {q.options.map((opt, oi) => (
                    <div key={oi} className="flex items-center gap-2">
                      ...
                      />
                      <span className="shrink-0 w-5 text-[10px] text-verde-600 font-bold">
              ...
              <div>
                <label className="block text-[10px] text-verde-600 mb-1">Explicación (opcional)</label>
                <input
                  type="text"
                  value={q.explanation ?? ''}
                  onChange={e => update(i, { explanation: e.target.value })}
                  placeholder="Se dice 'grazie' (pronunciado 'grátsie')"
...
export function ActivitiesManager() {
  ...
  async function save() {
    ...
    if (form.type === 'quiz') {
      const qs = (form.content as QuizContent).questions ?? []
    ...
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <span className="text-xs text-verde-600 bg-verde-950/40 border border-verde-900/30 px-2 py-1 rounded-lg">
            {items.length} attività
          </span>
        </div>
        <button
          onClick={() => openForm()}
          className="flex items-center gap-2 px-4 py-2 rounded-xl bg-verde-700 hover:bg-verde-600 text-white text-sm font-semibold transition-colors"
        >
          ...
          <div className="w-full max-w-lg bg-bg-dark border border-verde-800/40 rounded-2xl shadow-2xl overflow-hidden">
            ...
            <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
              <div>
                ...
                <input
                  value={form.title}
                  onChange={e => setForm(f => f ? { ...f, title: e.target.value, slug: f.id ? f.slug : slugify(e.target.value) } : f)}
              ...
              <div className="grid grid-cols-3 gap-3">
                <div>
                  ...
                  <select
                    value={form.type}
                    onChange={e => setForm(f => f ? { ...f, type: e.target.value, content: {} } : f)}
                ...
                <div>
                  ...
                  <select
                    value={form.level}
                    onChange={e => setForm(f => f ? { ...f, level: e.target.value } : f)}
                    className="w-full px-3 py-2 rounded-xl bg-verde-950/50 border border-verde-800/40 text-verde-200 text-sm focus:outline-none"
                  >
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={e => setForm(f => f ? { ...f, plan_required: e.target.value } : f)}
              ...
              {form.type === 'quiz' && (
                <QuizBuilder
                  ...
                  onChange={qs => setForm(f => f ? { ...f, content: { questions: qs } } : f)}
                />
              )}
                ...
                <div>
                  ...
                  <div
                    ...
                    className={`rounded-xl border-2 border-dashed transition-colors cursor-pointer ${
                      dragOver ? 'border-amber-500 bg-amber-950/20' : 'border-verde-800/40 bg-verde-950/20 hover:border-verde-700/60'
            ...
            <div className="flex gap-3 px-6 py-4 border-t border-verde-900/30">
              <button
                ...
                className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-verde-700 hover:bg-verde-600 text-white font-semibold text-sm disabled:opacity-50"
              >
              ...
              <button
                ...
                className="px-5 py-2.5 rounded-xl border border-verde-800/40 text-verde-400 text-sm"
              >
                Cancelar
              </button>
            </div>
      ...
      {loading ? (
        <div className="flex items-center justify-center py-20">
          <Loader2 size={24} className="animate-spin text-verde-600" />
        </div>
      ) : items.length === 0 ? (
        <div className="text-center py-20 text-verde-600">
          <Gamepad2 size={36} className="mx-auto mb-3 text-verde-800" />
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-verde-900/30">
                {['Título', 'Tipo', 'Nivel', 'Plan', 'Estado', ''].map(h => (
            ...
            <tbody className="divide-y divide-verde-900/10">
              {items.map(item => (
                <tr key={item.id} className="hover:bg-verde-950/20">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {item.file_url && (
                        <a href={item.file_url} target="_blank" rel="noopener noreferrer"
                          className="text-amber-600 hover:text-amber-400 transition-colors" title="Archivo adjunto">
                  ...
                  <td className="px-4 py-3">
                    <span className={`px-2 py-0.5 rounded text-xs font-bold ${LEVEL_COLORS[item.level]}`}>{item.level}</span>
                  </td>
                  <td className="px-4 py-3 text-xs text-verde-400 capitalize">{item.plan_required}</td>
                  <td className="px-4 py-3">
                    <button onClick={() => toggleStatus(item)} className="flex items-center gap-1 text-xs">
                      {item.status === 'published'
                        ? <><ToggleRight size={15} className="text-verde-400" /><span className="text-verde-400">Pub</span></>
                        : <><ToggleLeft size={15} className="text-verde-700" /><span className="text-verde-600">Draft</span></>}
                    </button>
                  </td>
                  <td className="px-4 py-3">
                    <div className="flex gap-2 justify-end">
                      <button onClick={() => openForm(item)} className="p-1.5 text-verde-600 hover:text-verde-300">
                        <Pencil size={13} />
                      </button>
                      <button onClick={() => del(item.id)} className="p-1.5 text-red-700 hover:text-red-400">
                        <Trash2 size={13} />
                      </button>
                    </div>
                  </td>
